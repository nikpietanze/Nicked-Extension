...
import { setTrackBtnReady, setTrackBtnSuccess, setTrackBtnError } from "../../amazon";
...
export default function(product: Product, saleTrackBtn: HTMLButtonElement) {
    ...
    const closeBtn: HTMLButtonElement | null = modal.querySelector("#nicked-modal-close");
    if (closeBtn) {
    ...
    const submitBtn: HTMLButtonElement | null = modal.querySelector("#nicked-email-submit");
    if (submitBtn) {
        submitBtn.addEventListener("click", async (e) => {
            e.preventDefault();
            const emailInput: HTMLInputElement | null = modal.querySelector("#nicked-email");
            ...
            if (emailInput) {
                ...
                if (isValid) {
                    ...
                    try {
                        const tracking = await product.isBeingTracked(email);
                        if (!tracking) {
