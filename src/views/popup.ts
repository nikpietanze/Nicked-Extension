...
interface Elements {
    ...
    settings: HTMLImageElement | null;
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (elements.logo) {
        elements.logo.src = chrome.runtime.getURL("/images/logo.webp");
    }

    elements.settings = document.querySelector("#settings");
    if (elements.settings) {
        elements.settings.src = chrome.runtime.getURL("/images/settings.svg");
    ...
    elements.app?.appendChild(elements.saleProducts);
    elements.app?.appendChild(elements.products);
...
function parseProducts() {
    ...
    for (let i = 0; i < user.products.length; i++) {
        ...
        productEl.innerHTML = `
            ...
            <span class="flex-auto text-stone-200 text-md font-semibold truncate">${product.name
            }</span>
            <img class="icon ${product.active
